...
// import { getProductName, getProductCategory, getProductImages, getProductPriceInfo } from "../../utils/productHelpers";
...
//   const productCat = getProductCategory(selectedProduct);
//   const images = getProductImages(selectedProduct);
//   const { price, finalPrice, discountPercentage } = getProductPriceInfo(selectedProduct);
//   const ratingValue = selectedProduct.ratings || 4.8;
//   const reviewsCount = selectedProduct.numOfReviews || 0;
//   useEffect(() => {
//     if (id) {
...
export const ProductDetails = () => {
  ...
  // ✅ guard clause first — selectedProduct null thakle ekhane e return kore dei,
  // eর niche kono selectedProduct.xxx access thakbe na direct
  if (isLoading || !selectedProduct) {
    return (
  ...
  // ✅ ekhon selectedProduct nishchit thakbe, tai ei niche shob derive kora safe
  const productName = getProductName(selectedProduct);
  const productCat = getProductCategory(selectedProduct);
  const images = getProductImages(selectedProduct);
  const { price, finalPrice, discountPercentage } = getProductPriceInfo(selectedProduct);
  ...
  const relatedProducts = products
    .filter((p) => getProductCategory(p) === productCat && p._id !== selectedProduct._id)
    .slice(0, 4);
  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      <Breadcrumb
        items={[
          { label: "Shop", path: "/shop" },
          { label: productCat, path: `/shop?category=${productCat}` },
          { label: productName },
        ]}
      />
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-soft grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col justify-between gap-6">
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400">
                {productCat} • {selectedProduct.brand}
              </span>
              <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 bg-emerald-100 dark:bg-emerald-950 px-2.5 py-0.5 rounded-full">
                <Check className="w-3.5 h-3.5" /> In Stock ({selectedProduct.stock})
              </span>
            ...
            <div className="flex items-center gap-4">
              <Rating value={ratingValue} reviewsCount={reviewsCount} size="md" />
              <span className="text-xs text-slate-400">•</span>
              <span className="text-xs text-slate-500 font-medium">
                Vendor: <strong className="text-slate-900 dark:text-slate-100">{selectedProduct.vendorName || "EcoBazar"}</strong>
              </span>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-soft flex flex-col gap-6">
        ...
        {activeTab === "description" && (
          <div className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed flex flex-col gap-3">
            <p>{selectedProduct.description}</p>
            {selectedProduct.AdditionalInfo && <p>{selectedProduct.AdditionalInfo}</p>}
            <ul className="list-disc list-inside space-y-1 text-slate-500">
              <li>100% Certified Organic & Non-GMO Project Verified.</li>
              <li>Responsibly harvested using sustainable eco-friendly farming practices.</li>
              <li>Kept in optimal cold-chain storage to guarantee crispness and flavor.</li>
        ...
        {activeTab === "reviews" && (
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800 rounded-2xl">
              ...
              <div>
                <Rating value={ratingValue} showCount={false} size="md" />
                <span className="text-xs text-slate-500">Based on {reviewsCount} verified buyers</span>
              </div>
            </div>

            {/* Sample review list - real review system nai ekhono, tai placeholder rakha holo */}
            <div className="flex flex-col gap-3 pt-2">
              <div className="p-4 border border-slate-100 dark:border-slate-800 rounded-2xl flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-900 dark:text-slate-100">Verified Buyer</span>
                ...
                <p className="text-xs text-slate-600 dark:text-slate-300">
                  Exceeded expectations! The product feels fresh and delivery was fast.
                </p>
              </div>
            </div>
          </div>
        ...
        {activeTab === "vendor" && (
          <div className="flex flex-col gap-2 text-xs sm:text-sm text-slate-600 dark:text-slate-300">
            <h4 className="font-bold text-slate-900 dark:text-slate-100">{selectedProduct.vendorName || "EcoBazar"}</h4>
            <p>Certified Organic Merchant. Specializing in daily fresh produce and zero-waste logistics.</p>
          </div>
        )}
      </div>

...
//         const title = getProductName(p).toLowerCase();
//         const category = getProductCategory(p).toLowerCase();
//         const brand = (p.brand || "").toLowerCase();
//         const sku = getProductSku(p).toLowerCase();
//         const term = searchTerm.toLowerCase();

//         return title.includes(term) || category.includes(term) || brand.includes(term) || sku.includes(term);
//       });
...
export const AdminProductsList = () => {
  ...
  return (
    <div className="flex flex-col gap-6 pb-12">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {isLoading ? (
                ...
                filteredProducts.map((p) => {
                  ...
                  const discountPercentage = Number(p.discountPrice) || 0;
                  const finalPrice =
                  ...
                  return (
                    <tr key={p._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                      ...
                      <td className="p-4">
                        {p.stock < 10 ? (
                          <Badge variant="danger">{p.stock ?? 0} (Low)</Badge>
                        ) : (
                          <Badge variant="success">{p.stock ?? 0} In Stock</Badge>
                        )}
                      </td>
                      <td className="p-4">
                        <Badge
      ...
      <Modal isOpen={!!deleteId} onClose={() => setDeleteId(null)} title="Confirm Delete Product">
        <div className="flex flex-col gap-4 text-xs text-slate-600 dark:text-slate-300">
          <p>
            Are you sure you want to delete this product? This action cannot be undone and will remove it from the live storefront.
          </p>
